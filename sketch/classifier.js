...
function setup() {
  ...
  const modelInfo = {
    model: 'tree/model.json',
    metadata: 'tree/model_meta.json',
    weights: 'tree/model.weights.bin',
  };
...
function draw() {
  ...
  if (pose) {
    ...
    for (let i = 0; i < pose.keypoints.length; i++) {
      let x = pose.keypoints[i].position.x;
      let y = pose.keypoints[i].position.y;
      if (poseLabel === 'W'||poseLabel === 'I' || poseLabel === 'T' || poseLabel === 'M') { // Change the condition based on the letter present
        fill(0, 255, 0);
      } else {
        fill(0); // Set keypoints to default color
      }
      stroke(255);
      ellipse(x, y, 16, 16);
    }
  ...
  text(poseLabel, 10, 10); // Display the pose label at (10, 10)
}
